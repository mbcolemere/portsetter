...
int main(int argc, char* args[]) {
    ...
    string temp = "";
    char* preLang = getenv("LANGUAGE");
    string lang = getLang(preLang);
    if (lang == "Error") {
        lang = "en";
        ...
        preLang = getenv("LC_ALL");
        lang = getLang(preLang);
        if (lang == "Error") {
            lang = "en";
            temp = getenv("LC_ALL");
            cout << "Bad language specification in environment variable";
            ...
            preLang = getenv("LC_MESSAGES");
            lang = getLang(preLang);
            if (lang == "Error") {
                lang = "en";
                temp = getenv("LC_MESSAGES");
                cout << "Bad language specification in environment variable";
                cout << " LC_MESSAGES=" + temp + " Using English." << endl;
            }
            else if (lang == "default") {
                preLang = getenv("LANG");
                lang = getLang(preLang);
    ...
    if (argc > 1) {
        ...
        if (argc == 4 && (flag == "-p" || flag == "--port")) {
            ...
            char* testVar = getenv(args[3]);
            if (testVar == nullptr) {
                cout << msg[2];
                usage(lang);
                return 2;
            }
            string envVar(testVar);
            if (envVar != "") {
                for (int i=0; i<envVar.length(); ++i) {
                    if (!isdigit(envVar[i])) {
                        cout << msg[1];
